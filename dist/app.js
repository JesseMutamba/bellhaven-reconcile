...
function render(){
  $('mode').textContent=state.mode==='demo'?'Demo workspace · local CRM':'Assessment CRM · connected';
  ...
  if(state.mode!=='demo') message('notice','Connected to your assessment CRM. Approving a proposal applies that change to your CRM copy.');
...
function renderDetail(){
  ...
  const {facility:f,before,changes,evidence,candidates,dependencies=[]}=p.plan;
  ...
  const survivorsHTML=dependencies.length?`<h3>Surviving CRM account</h3><table><thead><tr><th>Account</th><th>Address and parent</th><th>Financial context</th></tr></thead><tbody>${dependencies.map(a=>`<tr><td>${escapeHTML(a.name)}<br>${escapeHTML(a.id)}<br>${escapeHTML(a.status)}</td><td>${escapeHTML(a.address)}<br>${escapeHTML(a.city)}, ${escapeHTML(a.state)} ${escapeHTML(a.zip)}<br>Parent: ${escapeHTML(a.parent_name||a.parent_id)}${a.parent_name?`<br>${escapeHTML(a.parent_id)}`:''}</td><td>Lifetime revenue: ${escapeHTML(money(a.lifetime_revenue))}<br>Outstanding AR: ${escapeHTML(money(a.outstanding_ar))}</td></tr>`).join('')}</tbody></table>`:'';
  const form=p.state==='pending'||['retryable','applying'].includes(p.state)||(state.mode==='live'&&p.state==='uncertain')?`<form id="review-form" class="review"><label for="reviewer">Reviewer name</label><input id="reviewer" name="reviewer" value="${escapeHTML(reviewer)}" required maxlength="120" autocomplete="name" placeholder="Your name"><label for="reason">Review note</label><textarea id="reason" name="reason" maxlength="2000" placeholder="Required when rejecting or recording an investigation">${escapeHTML(note)}</textarea><div class="actions">${p.state==='pending'?`<button type="submit" class="primary" name="decision" value="${p.kind==='investigate'?'reviewed':'approve'}" ${busy||(!state.writes_available&&p.kind!=='investigate')?'disabled':''}>${p.kind==='investigate'?'Record investigation':p.kind==='chow'?'Approve successor account':'Approve change'}</button><button type="submit" name="decision" value="reject" ${busy?'disabled':''}>Reject proposal</button>`:`<button type="submit" class="primary" name="decision" value="retry" ${busy?'disabled':''}>${state.mode==='live'?'Check result / resume approved change':'Retry approved operation'}</button>`}</div></form>`:`<div class="decision"><strong>${escapeHTML(p.state.replaceAll('_',' '))}</strong><p>${escapeHTML(p.reviewer||'System')} · ${escapeHTML(date(p.decided_at||p.created_at))}</p>${p.reason?`<p>${escapeHTML(p.reason)}</p>`:''}</div>`;
  $('detail').innerHTML=`<div class="detail-head"><span class="badge ${p.kind}">${labels[p.kind]}</span><h2>${escapeHTML(p.title)}</h2><p>${escapeHTML(f.address)}<br>${escapeHTML(f.city)}, ${escapeHTML(f.state)} ${escapeHTML(f.zip)}${before?` · CRM ${escapeHTML(before.id)}`:''}</p></div><div class="detail-body">${p.kind==='chow'?'<div class="billing"><strong>Keep the historical account intact</strong>Create a new facility account under Bellhaven, then link the old account to it. Its existing parent, revenue history, balance, name, and status remain unchanged.</div>':''}${before?`<div class="finances"><span>Lifetime revenue<b>${escapeHTML(money(before.lifetime_revenue))}</b></span><span>Outstanding AR<b>${escapeHTML(money(before.outstanding_ar))}</b></span>${p.kind==='chow'?`<span>Preserved parent<b>${escapeHTML(before.parent_id)}</b></span>`:''}</div>`:''}${rows?`<h3>${p.kind==='chow'?'New successor account':'Proposed changes'}</h3><table><thead><tr><th>Field</th><th>Current</th><th>Proposed</th></tr></thead><tbody>${rows}</tbody></table>`:''}${candidatesHTML}${survivorsHTML}<h3>Supporting evidence</h3><ul class="evidence">${evidence.map(e=>`<li>${escapeHTML(e)}</li>`).join('')}</ul>${source}<div class="source-meta">Snapshot run ${p.run_id} · captured ${escapeHTML(date(p.created_at))}${f.source_sha256?` · SHA-256 ${escapeHTML(f.source_sha256.slice(0,12))}…`:''}</div>${p.error?`<div class="error">${escapeHTML(p.error)}</div>`:''}${form}<div id="audit" class="source-meta"></div></div>`;
